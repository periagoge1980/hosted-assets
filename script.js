...
window.onload = function () {
  fetchDataAndUpdate();
  document.getElementById('currentCountry').addEventListener(
    'change',
    function () {
      ...
      country => {
        return getCountryCode(country).then(
          countryCode => {
            ...
            return fetch(apiUrl).then(response => response.json()).then(
              apiData => {
                const gniPerCapita = apiData[1][0].value;
...
function getCountryCode(country_name) {
  return fetch('countryCode.json').then(response => response.json()).then(
    data => {
